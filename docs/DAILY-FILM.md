...
- `appKey` and `modeKey` to route Signal Hunt, Waypoint Rally, Waypoint Wings, or Treasure Hunter;
...
| Treasure Hunter | `daily-treasure-hunt` | Artifact discovery | Detector routes, reveals, and full-clear paths |
...
Each app shell injects its own replay-visual factory into the director. Signal Hunt supplies the hover car, Waypoint Rally its rally car, Waypoint Wings its aircraft, and Treasure Hunter its explorer, while timing, camera direction, overlay branding, and ReplayKit capture stay shared.
