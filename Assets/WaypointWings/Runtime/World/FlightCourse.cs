    ...
    public static class FlightCourseGenerator
    {
        public static FlightCourseLayout Generate(DailyChallenge challenge)
        {
            ...
            for (var index = 0; index < challenge.collectibleCount; index++)
            {
                ...
                var z = -82f + index * 22f;
                var x = Mathf.Sin(phase + progress * Mathf.PI * 3.25f) * random.Range(25f, 42f) + random.Range(-5f, 5f);
                var y = 24f + Mathf.Sin(phase * 0.7f + progress * Mathf.PI * 4.5f) * 11f + random.Range(-3f, 4f);
                var position = new Vector3(x, Mathf.Clamp(y, 11f, 48f), z);
                layout.gates.Add(new FlightGateDefinition
                {
                    id = $"gate-{index + 1:D2}",
                    position = position,
                    radius = random.Range(5.2f, 7.2f),
                    styleIndex = index % 4
                });
            }
        ...
        private static void GenerateEnvironment(FlightCourseLayout layout, DailyChallenge challenge, DeterministicRandom random)
        {
            ...
            for (var index = 0; index < 22; index++)
            {
                ...
                var position = gate.position + new Vector3(random.Range(-45f, 45f), -gate.position.y + random.Range(0f, 5f), random.Range(-18f, 18f));
                if (archipelago)
                {
                    layout.islands.Add(new SkyIslandDefinition
                    {
                        position = position,
                        scale = new Vector3(random.Range(9f, 24f), random.Range(2.2f, 6f), random.Range(9f, 24f)),
